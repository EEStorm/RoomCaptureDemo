...
@interface CD3DGSVideoListViewController () <UITableViewDelegate, UITableViewDataSource>
...
- (void)loadVideos {
    self.videos = [CD3DGSCameraService shared].recordedVideos;
...
- (void)confirmDeleteAll {
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"删除" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        [[CD3DGSCameraService shared] deleteAllVideos];
        [weakSelf loadVideos];
    }]];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        NSURL *url = self.videos[indexPath.row];
        [[CD3DGSCameraService shared] deleteVideoAtURL:url];
        [self loadVideos];
    }
}
