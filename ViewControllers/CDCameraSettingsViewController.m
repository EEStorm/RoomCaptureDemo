...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            cell.detailTextLabel.text = @"超广角 (0.5x)";
        }
    } else if (indexPath.section == 1) {
        UISlider *slider = [[UISlider alloc] init];
        slider.minimumValue = 2700;
        slider.maximumValue = 7500;
        slider.value = self.whiteBalance;
        slider.translatesAutoresizingMaskIntoConstraints = NO;
        [slider addTarget:self action:@selector(whiteBalanceChanged:) forControlEvents:UIControlEventValueChanged];
        [cell.contentView addSubview:slider];
        [NSLayoutConstraint activateConstraints:@[
            [slider.centerYAnchor constraintEqualToAnchor:cell.contentView.centerYAnchor],
            [slider.leftAnchor constraintEqualToAnchor:cell.contentView.leftAnchor constant:120],
            [slider.rightAnchor constraintEqualToAnchor:cell.contentView.rightAnchor constant:-60],
        ]];
        ...
        slider.value = self.shutterSpeed;
        slider.translatesAutoresizingMaskIntoConstraints = NO;
        [slider addTarget:self action:@selector(shutterSpeedChanged:) forControlEvents:UIControlEventValueChanged];
        [cell.contentView addSubview:slider];
        [NSLayoutConstraint activateConstraints:@[
            [slider.centerYAnchor constraintEqualToAnchor:cell.contentView.centerYAnchor],
            [slider.leftAnchor constraintEqualToAnchor:cell.contentView.leftAnchor constant:120],
        ...
        if (self.shutterSpeed >= 1) {
            ...
            cell.textLabel.text = [NSString stringWithFormat:@"1/%.0f", 1.0/self.shutterSpeed];
        }
    } else if (indexPath.section == 3) {
        UISlider *slider = [[UISlider alloc] init];
        slider.minimumValue = 50;
        slider.maximumValue = 2000;
        slider.value = self.iso;
        slider.translatesAutoresizingMaskIntoConstraints = NO;
        [slider addTarget:self action:@selector(isoChanged:) forControlEvents:UIControlEventValueChanged];
        [cell.contentView addSubview:slider];
        [NSLayoutConstraint activateConstraints:@[
            [slider.centerYAnchor constraintEqualToAnchor:cell.contentView.centerYAnchor],
            [slider.leftAnchor constraintEqualToAnchor:cell.contentView.leftAnchor constant:120],
...
- (void)whiteBalanceChanged:(UISlider *)slider {
    self.whiteBalance = roundf(slider.value / 100) * 100;
    [self.tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:1]] withRowAnimation:UITableViewRowAnimationNone];
}

- (void)shutterSpeedChanged:(UISlider *)slider {
    self.shutterSpeed = roundf(slider.value);
    [self.tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:2]] withRowAnimation:UITableViewRowAnimationNone];
}

- (void)isoChanged:(UISlider *)slider {
    self.iso = roundf(slider.value / 10) * 10;
